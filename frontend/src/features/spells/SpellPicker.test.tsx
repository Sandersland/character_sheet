...
describe("SpellPicker", () => {
  ...
  it("shows a distinct misconfiguration message when every group's eligible pool is empty", () => {
    render(
      <SpellPicker
        groups={[
          { key: "cantrips", label: "Cantrips", options: [], selectedIds: [], cap: 1, onToggle: vi.fn() },
        ]}
      />,
    );
    expect(screen.getByText(/no spells are available to choose here/i)).toBeInTheDocument();
    expect(screen.getByText(/configuration problem/i)).toBeInTheDocument();
    expect(screen.queryByText("No spells match your search.")).not.toBeInTheDocument();
  });
