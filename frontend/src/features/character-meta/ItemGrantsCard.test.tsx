...
describe("ItemGrantsCard (#529)", () => {
  it("renders nothing when there are no item grants", () => {
    const { container } = renderWithCharacter(
      <ItemGrantsCard />,
      makeCharacter({ resistances: [], grantedAdvantages: [] }),
    );
    expect(container).toBeEmptyDOMElement();
  });

  it("renders nothing when the grant fields are undefined", () => {
    const { container } = renderWithCharacter(<ItemGrantsCard />, makeCharacter({}));
    expect(container).toBeEmptyDOMElement();
  });
  ...
  it("shows a resistance with its item source, using the label helper (not a raw key)", () => {
    renderWithCharacter(
      <ItemGrantsCard />,
      makeCharacter({ resistances: [{ damageType: "fire", source: "Ring of Fire Resistance" }] }),
    );
