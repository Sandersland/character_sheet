...
describe("useCurrentCharacter", () => {
  ...
  // A cache write must reach every consumer via the query observer, not a
  ...
  // TanStack Query's structural sharing keeps the OLD reference when a write
  // is deeply equal to what's cached — a consumer memoised on identity must
  // not re-render for a no-op write.
  ...
  // A cache write must reach every consumer of the same id, not just
  // whichever component triggered the mutation.
