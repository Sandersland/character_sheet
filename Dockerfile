...
RUN npm run build --workspace @character-sheet/contracts \
 && node --input-type=module -e "import('@character-sheet/contracts').then(m => { const s = Object.keys(m).filter(k => k.endsWith('Schema')); if (s.length < 16 || s.some(k => typeof m[k]?.parse !== 'function')) { console.error('contracts barrel failed to load:', s.length, 'schemas'); process.exit(1); } console.log('contracts:', s.length, 'schemas loaded'); })"
