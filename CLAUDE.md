...
**Rules edition: 2024 (SRD 5.2).** SRD 5.2 is the canonical rules text; the 2024 PHB fills non-SRD content. Never settle a rules or mechanics question from the 2014 PHB/SRD 5.1 — code still encoding 2014 rules is legacy under migration (epic #1126), not precedent to match (drop this migration clause when #1126 closes). Rules citations in comments reference the 2024 books (`PHB'24 p. NN` / `SRD 5.2`); stale 2014 citations are updated by whichever PR touches that rule, never in a sweep.
