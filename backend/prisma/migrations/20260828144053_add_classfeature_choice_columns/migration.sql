...
-- and 20260828134438_add_resource_on_initiative: that FK is hand-written onto
-- a deliberately relationless scalar (Spell.catalogEntryId has no Prisma
-- @relation), so every diff proposes dropping it — applying that would
-- destroy real integrity.

-- AlterTable
ALTER TABLE "ClassFeature" ADD COLUMN     "choiceCatalogSource" TEXT,
ADD COLUMN     "choiceCountTiers" JSONB,
ADD COLUMN     "choiceKey" TEXT,
ADD COLUMN     "choiceLabel" TEXT;
