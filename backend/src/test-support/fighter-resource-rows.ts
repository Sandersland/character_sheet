// Test-only helper (#1528): Fighter's base resource pools (Second Wind/Action
// Surge/Indomitable) moved from a code resourceFn (keyed on className alone)
// onto ClassFeature rows tied to a SPECIFIC CharacterClass row's id. A test
// fixture that creates its own bespoke "Test Fighter" CharacterClass row
// (per testing.md — never mutate the real seeded catalog) therefore has NO
// matching ClassFeature children, so Second Wind/Action Surge silently
// vanish for that fixture unless it seeds them itself. This is that seed.
//
