// ?edition= is mandatory: absent and unrecognized both 400, so both assert their message since status alone can't distinguish them.
...
describe("GET /api/feats — edition resolution (#1306)", () => {
  ...
  // SRD 5.2 half-feat vs SRD 5.1 flat Strength 13+ prerequisite (#1310).
  it("each edition resolves Grappler to its OWN row, not a shared one", async () => {
    ...
    // abilityIncrease defaults to 0, not NULL — 0 is the no-half-feat-bump wire value.
...
describe("GET /api/feats?asiLevel= — server-side ASI eligibility (#1438)", () => {
  ...
  it("gates the seeded General feats on their explicit levelPrerequisite 4", async () => {
    ...
    // Absolute counts guard against a filter that vacuously drops everything but the named rows.
  ...
  it("never offers an Origin or Fighting Style feat at any level", async () => {
    for (const asiLevel of [1, 3, 4, 18, 19, 20]) {
      ...
      expect(names(rows)).not.toContain("Magic Initiate");
      expect(names(rows)).not.toContain("Archery");
...
describe("GET /api/feats?asiLevel= — 2014 general/origin feats (#1310)", () => {
  ...
  it("gates the 2014 general feats on the `?? 4` category default — the AC that no fixture-only test can give", async () => {
    ...
    // All 26 rows share a NULL levelPrerequisite — the `?? 4` default applies uniformly, not per-row.
    expect(atThree).toHaveLength(0);
    expect(atFour).toHaveLength(26);
  });
});

// PHB'14 p. 72 (Fighter) / p. 82 (Paladin) / p. 91 (Ranger) = SRD 5.1 (#1495); gated via fightingStyleFeatOfferedForClasses.
describe("GET /api/feats?classes= — 2014 Fighting Style class gate (#1495)", () => {
  ...
  // featOfferedForAsiSlot strips every fighting_style row before classes filters, so combining used to silently no-op instead of erroring (#1495).
  ...
  // A repeated ?classes= key parses as an array under Express's query parser, not a string — the error must name that reason (#1495).
