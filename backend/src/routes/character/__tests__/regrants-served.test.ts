...
describe("GET /api/characters/:id — availableActions[].regrants", () => {
  for (const edition of EDITIONS) {
    it(`${edition}: every regranted key is served as a universal action still costing an action`, async () => {
      const rogueL2 = await makeRogue(edition, `Regrants Rogue L2 ${edition}`, 2);
      const thiefL3 = await makeRogue(edition, `Regrants Thief L3 ${edition}`, 3, "Thief");
      const served = await servedUniversals(edition);

      const seen = new Set<string>();
      for (const id of [rogueL2, thiefL3]) {
      ...
      expect([...seen].sort()).toEqual(["dash", "disengage", "hide", "useObject"]);
  ...
  it("Fast Hands is a Thief-only, level-3 bonus action in both editions", async () => {
    for (const edition of EDITIONS) {
      const thief = await availableActions(await makeRogue(edition, `Fast Hands Thief ${edition}`, 3, "Thief"));
      const plain = await availableActions(await makeRogue(edition, `Fast Hands Rogue ${edition}`, 2));
