...
// #1285: one rule (subclassGateLevel) resolved through the edition seam — a reconciler, its clamp-on-read, and write-side validation must all resolve to the SAME edition's function.
// This fixture class seeds no ClassFeature rows, so deriveResources returns null and the test exercises only the catalog-driven subclassGateLevel.
...
// Cleanup filters on the uniquely-prefixed fixture names, never on classId — if beforeAll threw, classId is undefined and Prisma reads that as "no filter", deleting every seeded Subclass row.
afterAll(async () => {
