...
// its Cha-modifier total IS now expressible as a formula tier and its
// longRest-then-short-or-long recharge shift is now expressible via
// resourceRechargeTiers too, but a level-conditional description (the die
// size and rest type are interpolated inline, #1528's no-second-string rule)
// still blocks it. This module is NOT deletable, for a reason that has
// nothing to do with the subclass gate — Bard is Ranger-shaped
// (lib/classes/ranger.ts), not Cleric-shaped: both `college of lore` and
// `college of valor` declare `grantLevel: 3`, which already equals
// subclassGateLevel's undefined fallback, so omitting either changes nothing
// about the 2014 gate. The resourceFn below is the ONLY reason this file
// survives.
//
