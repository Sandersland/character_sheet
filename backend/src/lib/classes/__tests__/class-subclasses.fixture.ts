...
// backend/prisma/seed/fighter-features.ts), keyed lowercase to match this
// file's own registry convention — the sibling of prisma/seed/class-
// features.ts's LITERAL_ROW_CLASSES (Title Case, "Fighter"). Kept as a SECOND
// constant rather than importing the prisma-side one directly: backend/
// tsconfig.json's `rootDir: "src"` makes any src file importing something
// under prisma/ a compile error (TS6059, verified empirically) — there is no
// single set both sides can share. Both must be updated together when a
// class's rows go literal; class-feature-parity.test.ts and feature-edition.
// test.ts key off THIS one so there is still only one hand-maintained list
// per side, never one per test.
export const LITERAL_ROW_CLASSES = new Set(["fighter"]);
