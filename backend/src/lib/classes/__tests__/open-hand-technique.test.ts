...
// character-serialize.ts's openHandTechniqueRider and this module's own cast
// guard import it. No numeric literal for the gate appears here; the
// boundary is derived from OPEN_HAND_TECHNIQUE_LEVEL itself, so mutating that
// one constant would flip both this assertion and the rider/guard together.
