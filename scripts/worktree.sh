...
#
#   ./scripts/worktree.sh create <branch> [--up]   create worktree, assign slot, write .env
#   ./scripts/worktree.sh up <branch>              docker compose up -d (build)
#   ./scripts/worktree.sh down <branch>            docker compose down (keep volumes)
#   ./scripts/worktree.sh ls                       table of worktrees, slots, URLs, status
#   ./scripts/worktree.sh rm <branch>              down -v + remove worktree + free slot
#
set -euo pipefail

# Repo root = parent of this script's dir. Resolve symlinks for safety.
...
write_env() {
  ...
  cat > "$path/.env" <<EOF
# Generated by scripts/worktree.sh — worktree '$branch', slot $slot.
# Gitignored. Regenerate with: ./scripts/worktree.sh create $branch
...
cmd_create() {
  ...
  if [ "$do_up" = 1 ]; then
    ...
    echo "  start it: ./scripts/worktree.sh up $branch"
  fi
}
...
cmd_ls() {
  ...
  if [ -z "$branches" ]; then
    echo "No worktrees. Create one: ./scripts/worktree.sh create <branch>"
    return
  fi
