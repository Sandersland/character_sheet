...
**A new subclass row** additionally needs a slug: add the new member to `SUBCLASS_SLUGS` and its identity to `SUBCLASS_IDENTITY` (`lib/classes/subclass-slug.ts`), the matching literal on the seed row (`prisma/seed/subclasses.ts`), and — unless its class is already row-migrated (`ROW_MIGRATED_CLASSES` in `seed-data.test.ts`) — its `SubclassDefinition` (`lib/classes/<class>.ts`) — the bijection tests in `seed-data.test.ts` fail with a diff if a required one is missing — four directional checks over the three sides (#1277).
