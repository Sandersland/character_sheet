...
A third step tightens the backend further (#692, live 2026-07-11): `fallow health -w backend --production --coverage backend/coverage/coverage-final.json` runs in **plain mode** (fails on *any* finding in scope), so **every backend production function must stay within the complexity ceilings** (`cyclomatic ≤20`, `cognitive ≤15`) **and the `maxCrap: 30` bar** — not just the CRITICAL-severity ones. This is strictly stronger than the `--gate new-only` audit: that only inspects the PR diff, whereas this catches a refactor that pushes an *existing* backend function over the bar. `--production` excludes test files (test complexity isn't gated). `--coverage` feeds fallow the real vitest Istanbul coverage (#717): the `test` job runs `npm run test:coverage -w backend`, uploads `backend/coverage/coverage-final.json` as an artifact, and the `fallow` job (`needs: test`) downloads it — so CRAP is **exact** (an untested complex function reports real high CRAP) rather than the export-reference estimate that inflated every modestly-complex function under `--production`. Because the artifact is produced and consumed on the same runner (identical `$GITHUB_WORKSPACE` paths), no `--coverage-root` remap is needed. Tightening the *repo-wide* floor past `critical` stays blocked on the frontend HIGH tier and is intentionally not attempted; this ratchet is backend-only. To reproduce the gate locally: `docker compose up db -d`, `npm run test:coverage -w backend` (see `docs/testing.md`), then `npx --no-install fallow health -w backend --production --coverage backend/coverage/coverage-final.json`.
