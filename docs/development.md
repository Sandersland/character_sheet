...
The fallow audit also runs **server-side**: CI's `fallow` job (a required check on PRs, #691) re-runs the same `fallow audit --gate new-only` against the PR base with the same pinned version, so a locally bypassed hook doesn't skip the boundary. When a finding is adjudicated as acceptable (e.g. an inherent validation ladder just over the bar), suppress it **inline** with `// fallow-ignore-next-line complexity` — the suppression lands in the diff where the PR review sees it, unlike a `--no-verify` bypass which leaves no trace.

Beyond the audit, the job runs **two per-workspace `fallow health` gates** (#718, tightened from the old single repo-wide `--min-severity critical` step) so that **no HIGH- or CRITICAL-severity function survives in either workspace** — the "critical→high" ratchet #692 aimed for. They're split per-workspace because each workspace's CRAP treatment differs:

- **Backend** (#717, live 2026-07-11): `fallow health -w backend --production --coverage backend/coverage/coverage-final.json` runs in **plain mode** (fails on *any* finding in scope), so **every backend production function must stay within the complexity ceilings** (`cyclomatic ≤20`, `cognitive ≤15`) **and the `maxCrap: 30` bar**. `--coverage` feeds fallow the real vitest Istanbul coverage: the `test` job runs `npm run test:coverage -w backend`, uploads `backend/coverage/coverage-final.json` as an artifact, and the `fallow` job (`needs: test`) downloads it — so CRAP is **exact** (an untested complex function reports real high CRAP) rather than the export-reference estimate that inflated every modestly-complex function under `--production`. Same-runner artifact (identical `$GITHUB_WORKSPACE` paths), so no `--coverage-root` remap. Reproduce locally: `docker compose up db -d`, `npm run test:coverage -w backend`, then `npx --no-install fallow health -w backend --production --coverage backend/coverage/coverage-final.json`.
- **Frontend** (#718): `fallow health -w frontend --production --min-severity high --max-crap 999` — a **complexity-ceiling** gate only. `--max-crap 999` drops the CRAP axis, so severity is driven purely by cyclomatic/cognitive. Frontend coverage is **not** wired in here: at ~69% coverage real CRAP would flag coverage debt, not complexity (it explodes the tier to ~39 findings), so enforcing frontend CRAP is deferred to a future issue (the frontend twin of #717) once coverage is raised. Reproduce locally: `npx --no-install fallow health -w frontend --production --min-severity high --max-crap 999`.

Both gates are strictly stronger than the `--gate new-only` audit (which only inspects the PR diff): they catch a refactor that pushes an *existing* function over the bar, and they run on pushes to `main`/`staging` too, so a direct push can't slip past. A function that crosses a bar fails CI until it's decomposed or carries a reviewed inline suppression.
