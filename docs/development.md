...
A third step tightens the backend further (#692, live 2026-07-11): `fallow health -w backend --production --max-crap 999` runs in **plain mode** (fails on *any* finding in scope), so **every backend production function must stay within the coverage-independent complexity ceilings** (`cyclomatic ≤20` and `cognitive ≤15`) — not just the CRITICAL-severity ones. This is strictly stronger than the `--gate new-only` audit: that only inspects the PR diff, whereas this catches a refactor that pushes an *existing* backend function over the bar. `--production` excludes test files (test complexity isn't gated); `--max-crap 999` drops the estimated-CRAP axis, which is unreliable without real coverage and inflates severity under `--production` (wiring real vitest coverage via `--coverage` to re-enable a meaningful CRAP bar is a tracked follow-up). Tightening the *repo-wide* floor past `critical` stays blocked on the frontend HIGH tier and is intentionally not attempted; this ratchet is backend-only.
