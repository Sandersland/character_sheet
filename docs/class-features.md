...
- An **Activation** block's `resolverKind: "toggle"` forks into `toggleActionsFromRow`, which synthesizes an activate/end action PAIR from one row via `endActionKey`, and routes the row's Buff/Immunity blocks through the activate side (`toggleRowOps`).
...
2. `classFeatureSeedSchema`, exercised via `assertSeedContentValid` (`backend/prisma/seed/__tests__/validate.test.ts`), structurally validates the row (including, per row, that a `choiceKey` is properly subclass-scoped via `choiceRowIsSubclassScoped`) and — over the whole `CLASS_FEATURES` array — rejects a duplicate pool/choice declaration (`assertNoDuplicatePoolDeclaringRows`/`assertNoDuplicateChoiceDeclaringRows`).
