...
| `lib/effects.ts` | The 5e effect model (dice + save + scaling), extracted from `Spell` so it describes any activated ability. `EffectSpec`/`EffectScaling`/`EffectColumns` types, `readEffectSpec(row)` (adapter over the flat effect columns — no schema change), `resolveEffectSpec(spec, effectiveStep, ctx)` → concrete dice `{count,faces,modifier}`, generalizing the scaling axis (`cantripLevel`/`slotUpcast`/`ki`). Adds the `buff` `EffectType` + `resolveBuffSpec(spec)` (adapts a buff effect into a `{target,modifier}` passive-modifier spec). Hand-mirrored on the frontend in `frontend/src/lib/effects.ts` (which returns the real `RollSpec`). |
...
| `lib/active-effects.ts` | Passive-modifier (buff) state for the `activeEffects` JSON column. `normalizeActiveEffectsMutable(json)` + `buffsByTarget(state)` shape/group the mutable state; `appendActiveBuffInTx()` (dedupes by `key`, replacing — always writes + logs `buffApplied`) and `clearBuffsForSourceInTx()` (drops buffs keyed by `sourceEntryId`, logs `buffCleared`) are the two tx helpers. Buffs ride concentration (no duration engine). Pure mutable state (not level-derived); analog to `lib/conditions.ts`. |
...
| `lib/session-summary.ts` | Pure aggregation (no Postgres): `computeSessionSummary()` folds a participant's `CharacterEvent` rows into a summary — items split into `itemsAcquired` (net gains) vs `itemsSold` (positive sale counts, never a negative acquisition); `computeCampaignRecap()` aggregates per-participant `ParticipantSummary`s into the campaign-level `CampaignRecap` (sum XP/spells/rounds/rolls, union acquired+sold items, sum `slotsSpent`, concat `featsOrAsis`, participant count, present-time). Derive-don't-persist. |
...
- `resources Json?` — mutable state only: `{ used, maneuversKnown, disciplinesKnown, toolProficienciesKnown, advancements, fightingStyle }`. Pool totals/die/recharge and all level-gated caps are derived at read time via `deriveResources()`; the persisted lists are clamped-on-read. `disciplinesKnown[].disciplineId` stores a GrantedAbility catalog uuid (`source: "discipline"`) — unchanged by the table rename.
- `conditions Json?` — `{ active: ConditionEntry[], exhaustion: number }`. Pure mutable state, not level-derived.
- `activeEffects Json?` — `{ buffs: ActiveBuff[] }` (`lib/active-effects.ts`). Cast-granted passive stat modifiers, each tagged with the granting concentration's `sourceEntryId` and deduped by `key` on re-cast. Applied generically by `castAbilityInTx` when an effect resolves to a buff (`resolveBuffSpec`, `lib/effects.ts`), cleared when that concentration ends (new-concentration replace, failed CON save, manual drop/forget). `serializeCharacter` sums them per target into the affected skill's optional `tempModifier` + `tempModifierSources` breakdown.
