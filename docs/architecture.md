...
| `lib/resources.ts` | `applyResourceOperations()` — spend/restore class resources, learn/forget maneuvers + tool profs, and learn/forget/swap Four Elements disciplines; persists `used` counts and known lists in `resources` JSON. Analog to `spellcasting.ts`. Discipline learn/swap enforce the derived cap + per-discipline min monk level; swap is gated to one per monk level via `DisciplineEntry.lastSwapLevel`. `applySpendResourceInTx` returns its `ResourceOpAudit` so callers (`ability-cost.ts`) can surface the spend label. |
