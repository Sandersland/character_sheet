...
| `lib/srd.ts` | **All 5e rules data**: alignments, skills, ability-modifier math, per-class spellcasting ability, `FULL_CASTER_SLOTS`, `deriveCreatedCharacter()`, `deriveSpellcasting()`, `deriveMulticlassSpellcasting()` (merged caster-level slot pool + separate Warlock Pact Magic; used by `serializeCharacter` for 2+ class entries), `deriveWeaponAttackBonus()`, `deriveWeaponDamage()` (grip-aware: versatile die when off-hand is free), `deriveArmorClass()` (best equipped body armor + Dex per category + shield; AC is derived, never a column), `multiclassPrerequisitesMet()` + `MULTICLASS_PREREQUISITES` (ability-score gate for multiclass level-up; thresholds also served on `GET /reference` per class). **This is the only permitted location for rules data.** |
...
`armorClass` is **not** a column: `serializeCharacter` derives it at read time via `deriveArmorClass()` from the best equipped body-armor `InventoryItem` snapshot + effective Dex (capped per armor category) + shield, then adds Defense fighting-style and feat `armorClass` bonuses. Equip/unequip armor (or change Dex) via the inventory/ability transactions and AC re-derives; it's absent from the `PATCH /characters/:id` allowlist.
