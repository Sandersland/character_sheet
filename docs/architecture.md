...
| `lib/granted-spells.ts` | 5e rules data for choice-less subclass-granted spells: pure `deriveGrantedSpells(className, subclass, level)` driven by the `SUBCLASS_GRANTED_SPELLS` table (Way of Shadow → Minor Illusion at L3). Derived at read time and merged into the spellcasting view — never persisted (`reconcileGrantedSpells` in `level-reconciliation.ts` is the defense-in-depth guard). |
