...
| `lib/resources.ts` | `applyResourceOperations()` — spend/restore class resources and learn/forget maneuvers + tool profs; persists `used` counts and known lists in `resources` JSON. Analog to `spellcasting.ts`. `applySpendResourceInTx` returns its `ResourceOpAudit` so callers (`ability-cost.ts`) can surface the spend label. |
