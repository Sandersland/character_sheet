...
│   │                    #   ReactionSlot/EffectManeuverStrip/LayOnHandsInput + LoadoutSwapRow (#733)
...
| `loadoutPicker.ts` | JSX-free core of `LoadoutSwapRow`'s per-hand picker (#789): `handContext` (live MAIN/OFF occupants + `actionsRemaining`), `handPickerOptions` (bag candidates deduped by name with ×N counts + a free Stow, each tagged `costsAction`/`disabledReason`), `handButtonDisabledReason`, `noLegalMove`. Mirrors `useLoadoutSwap`'s `buildSwapOps` cost rule — a swap costs the Action when it stows a held item (the target hand, or the other hand for a two-handed weapon). Keeps the component under fallow thresholds. |
