...
| `turnActionPlan.ts` | Pure planner for TurnHub action-slot clicks (#613): `planActionClick(resolver, character)` → an `ActionClickPlan` (`{ consumeSlot, openResolution, send: "none"\|"plain"\|"healRoll", healRoll? }`) that `useTurnActions.handleActionClick` interprets. Keeps the per-kind branching testable and out of the hook's dispatch closure. |
