...
| `Popover` | Owned-trigger **disclosure** popover for read-only detail panels (vs `DropdownMenu`'s menu semantics — no menuitems, no roving focus). Trigger gets `aria-haspopup="dialog"`/`aria-expanded`; panel is `role="dialog"` (focused on open), Esc closes and refocuses the trigger, click-outside and re-click close. No portal — `relative`-anchored. Props: `trigger`, `label` (trigger + dialog accessible name), `children` (a `ReactNode`, or a render fn `(close) => ReactNode` so panel controls can dismiss the popover), `align?` (`left`\|`right`, default `left`), `className?`, `triggerClassName?` (style the owned button, e.g. as a stat tile), `onClose?` (fires on every open→closed transition). |
