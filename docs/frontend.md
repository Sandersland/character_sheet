...
| `spellPicker.ts` | Pure InlineSpellPicker selection/slot predicates (`availableSlotLevels`, `availableSlotsForSpell`, `resolvedSlot`, `filterCastableSpells`, `sortSpells`, `spellRestrictionFlags`, `slotRestrictionHint`). Note the like-named `availableSlotsForSpell` lives here (InlineSpellPicker) **and** in `spellList.ts` (SpellsSection) — different call sites, don't cross-import. |
| `spellList.ts` | Pure SpellsSection list derivation (#606). `deriveSpellList(character)` returns a `SpellListDerivation` (available slot/arcanum levels, learned catalog spell ids, level-sorted spells + present levels, dismissible while-active spell buffs, single-class-warlock Pact-Magic labeling); `availableSlotsForSpell(spell, slotLevels, arcanaLevels)` picks the castable slot levels for one spell. Sibling of `spellPicker.ts` (the session-mode equivalent) — this file backs the out-of-session sheet section. |
...
| `turnRules.ts` | 5e turn economy — universal action lists + `canTwoWeaponFight(inventory, fightingStyle?)` (two equipped light weapons → TWF; the **Two-Weapon Fighting style** relaxes the light restriction, #732). (Extra Attack counts are server-derived; read `character.attacksPerAction`.) |
