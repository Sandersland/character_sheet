...
Specs live in `frontend/e2e/`; run via `npm run e2e` (→ `docker compose --profile e2e run --rm e2e`, a pinned Playwright image on host networking that derives its base URL from `FRONTEND_PORT`, so it works against the main checkout or any worktree slot).

**Start `npm run dev` first.** The runner used to `depends_on` the frontend container; that container is gone (#1458), so it now points host networking at servers it does not start. A suite that fails instantly with connection refused means the dev servers aren't up.
