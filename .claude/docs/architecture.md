...
| `lib/srd.ts` | **All 5e rules data**: alignments, skills, ability-modifier math, `SPELLCASTING_ABILITY`, `FULL_CASTER_SLOTS`, `deriveCreatedCharacter()`, `deriveSpellcasting()`, `deriveWeaponAttackBonus()`, `deriveWeaponDamage()` (grip-aware: versatile die when off-hand is free), `multiclassPrerequisitesMet()` (ability-score gate for multiclass level-up). **This is the only permitted location for rules data.** |
