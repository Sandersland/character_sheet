...
| `lib/auth/providers/` | OAuth provider registry, one module per provider. `index.ts` holds the `PROVIDER_DEFINITIONS` array + `enabledProviders()`/`getProvider(id)`, which read creds from **live env** so a provider can be toggled on without a re-import (only id+secret-both-set providers are enabled). `types.ts` defines `ProviderDefinition`/`AuthProvider`/`NormalizedProfile`. Each provider (e.g. `google.ts`) exports a single `ProviderDefinition` — descriptor + its own `clientIdEnv`/`clientSecretEnv` names + a `mapProfile` (Google's zod-parses userinfo and nulls an unverified email). **Adding a provider = a new module + one entry in `PROVIDER_DEFINITIONS`; no shared code changes.** |
...
All three auth children cascade-delete with their `User`. Resolution today: `lib/owner.ts` `resolveBootstrapOwnerId()` returns a single bootstrap owner (upsert by `BOOTSTRAP_OWNER_EMAIL` if set, else reuse-earliest-or-mint) — a placeholder until **#100** wires real sign-in and **#101** enforces per-owner access. `GET /characters` parses `?owner=` but does **not** apply it yet (`TODO(#101)`). The migration backfills any pre-existing characters onto one minted bootstrap user before enforcing `NOT NULL`.
