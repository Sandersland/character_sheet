...
if (process.argv[2] === "--test") {
  ...
  const CASES = [
    ...
    ["Worker", "docker compose exec -T backend sh -c 'cd /app/backend && npx vitest run' && gh pr create", 2],
    ["Reviewer", "docker compose exec -T backend sh -c 'cd /app/backend && npx vitest run'", 0],
