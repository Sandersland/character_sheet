...
#
# The isolation that matters is the DATABASE and the port block, not a container
# per process: since #1458 `up` starts Postgres and prepares its schema, and the
# app runs on the host from the worktree's own node_modules.
#
#   ./.claude/skills/worktree/worktree.sh create <branch> [--up]   create worktree, assign slot, .env, npm ci
#   ./.claude/skills/worktree/worktree.sh up <branch>              start + migrate + seed this worktree's database
...
write_env() {
  ...
  local gen="# Generated by .claude/skills/worktree/worktree.sh — worktree '$branch', slot $slot.
# Gitignored. Regenerate with: ./.claude/skills/worktree/worktree.sh create $branch"

  # Compose's own env. Only the database and the e2e runner are containers now,
  # so this file has shrunk to what those two read (#1458); FRONTEND_PORT stays
  # because e2e derives E2E_BASE_URL from it.
  cat > "$path/.env" <<EOF
$gen
...
EOF
  ...
  # The dev containers used to inject these by service name. Host-run servers get
  # them from the per-workspace .env instead — loaded explicitly, see #1463.
  cat > "$path/backend/.env" <<EOF
$gen
DATABASE_URL=postgresql://character_sheet:character_sheet@localhost:$pg/character_sheet
PORT=$backend
ALLOW_DEV_LOGIN=true
RATE_LIMIT_DISABLED=true
EOF

  # Vite reads these at CONFIG time via loadEnv, which is why they live here and
  # not in the compose file: the proxy target and the pinned port are what keep a
  # slot's SPA talking to its own backend instead of the main checkout's.
  cat > "$path/frontend/.env" <<EOF
$gen
FRONTEND_PORT=$frontend
...
cmd_up() {
  ...
  local slot; slot="$(slot_of "$branch")"
  echo "Starting '$branch' database…"
  ( cd "$path" && docker compose up -d db )

  # The dev backend image ran migrate+seed on every boot; with it retired this is
  # the only thing that prepares a worktree's database, and a schema-less DB fails
  # in ways that read like application bugs (#1458). Wait for Postgres first —
  # compose reports the container started well before it accepts connections.
  local pg; pg="$(port_for "$slot" 5432)"
  local i
  for i in $(seq 1 60); do
    ( cd "$path" && docker compose exec -T db pg_isready -U character_sheet >/dev/null 2>&1 ) && break
    [ "$i" = 60 ] && die "database on port $pg never became ready"
    sleep 1
  done
  echo "Applying migrations and seeding…"
  ( cd "$path/backend" && npx prisma migrate deploy >/dev/null && npx prisma db seed >/dev/null ) \
    || die "migrate/seed failed for '$branch'"

  print_ports "$branch" "$slot"
  # Deliberately printed, not spawned: dev servers are long-lived foreground
  # processes with output worth watching, and a backgrounded pair would outlive
  # this script with nothing owning them.
  echo "  run the app: (cd $path && npm run dev)"
}
...
# Every volume compose has EVER created for this stack, so an orphan can be
# mapped back to the project that made it. The backend_/frontend_ names outlived
# their services in #1458 and stay listed on purpose — dropping them would strand
# every volume those containers left behind, with no `rm` able to reach them.
